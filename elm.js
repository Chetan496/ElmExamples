...
Elm.LetExpressions.make = function (_elm) {
   "use strict";
   _elm.LetExpressions = _elm.LetExpressions || {};
   if (_elm.LetExpressions.values)
   return _elm.LetExpressions.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "LetExpressions",
   $Basics = Elm.Basics.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   var hypotenuse = function () {
      ...
      var y = 4;
      var x = 3;
      return $Basics.sqrt(square(x) + square(y));
   }();
   var main = $Graphics$Element.show(_L.fromArray([hypotenuse]));
   _elm.LetExpressions.values = {_op: _op
